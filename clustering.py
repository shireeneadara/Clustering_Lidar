...
def split_cloud(cloud):
  ...
  downsampled_cloud = do_voxel_grid_filter(point_cloud = cloud, LEAF_SIZE = 0.2)
  ...
  filtered_cloud = do_passthrough_filter(point_cloud = downsampled_cloud, 
                                         name_axis = 'z', min_axis = -0.8, max_axis = 0.8)
	
  filtered_cloud = do_passthrough_filter(point_cloud = filtered_cloud, 
                                         name_axis = 'x', min_axis = -10.0, max_axis = 100.0)
  filtered_cloud = do_passthrough_filter(point_cloud = filtered_cloud, 
                                         name_axis = 'y', min_axis = -10.0, max_axis = 20.0)
  # Separate the table from everything else
  table_cloud, objects_cloud = do_ransac_plane_segmentation(filtered_cloud, max_distance = 0.2)
...
def pcl_callback(pcl_msg):
  ...
  clusters = get_clusters(colorless_cloud, tolerance = 1.0, min_size = 150, max_size = 25000)
...
if __name__ == '__main__':
  ...
  # Create Subscribers
  subscriber = rospy.Subscriber("/velodyne_points", PointCloud2, pcl_callback, queue_size = 1)
